...
(*Test Code*)
      ...
      let rec matchLHS lhs = (match lhs with
      | A.IdentExp(symbol, _) -> symbol
      | A.FstExp(exp, _) -> (match exp with
        | A.IdentExp(symbol,_) -> Symbol.symbol ("fst" ^ (Symbol.string_of_symbol symbol))
        | A.ArrayIndexExp(symbol,_,_) -> Symbol.symbol ("fst[]" ^ (Symbol.string_of_symbol symbol))
        | A.FstExp(exps,_) -> matchLHS exps
        | A.SndExp(exps,_) -> matchLHS exps
        | _ -> Symbol.symbol "what"
        )
      | A.SndExp(exp,_) -> (match exp with
        | A.IdentExp(symbol,_) -> Symbol.symbol ("snd" ^ (Symbol.string_of_symbol symbol))
        | A.ArrayIndexExp(symbol,_,_) -> Symbol.symbol ("snd[]" ^ (Symbol.string_of_symbol symbol))
        | A.FstExp(exps,_) -> matchLHS exps
        | A.SndExp(exps,_) -> matchLHS exps
        | _ -> Symbol.symbol "what"
        ))
...
let handle_semantic_error lexbuf =
  ...
  let rec eval singleStmt = (match singleStmt with
    | A.SeqStmt statement -> evalAll statement
    | A.SkipStmt(_) ->  ()
    ...
    | A.BlockStmt(statment, _) ->
         eval statment)
  and evalAll listStatement = (match listStatement with
     | [] -> ()
     | h::r ->( let () = eval h in
               let () = evalAll r in () ))
