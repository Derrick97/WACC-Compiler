...
let size_of_type = function
  | A.CharTy | A.BoolTy -> 1
  | A.IntTy -> 4
  | A.StringTy | A.PairTy _ | A.NullTy
  | A.PairTyy | A.ArrayTy _ -> 4 (* addresses *)
      ...
      | (dst::rest) -> begin
          ...
          | A.BinOpExp    (exp, binop, exp') -> begin
              ...
              (match binop with
               ...
               | A.GeOp -> begin  (* FIXME we can use table driven methods here *)
                   [cmp dst oper;
                    mov ~cond:GE dst (imm 1);
                    mov ~cond:LT dst (imm 0)]
                 end
               | A.GtOp -> begin
                   [cmp dst oper;
                    mov ~cond:GT dst (imm 1);
                    mov ~cond:LE dst (imm 0)]
                 end
               | A.LeOp -> begin
                   [cmp dst oper;
                    mov ~cond:LE dst (imm 1);
                    mov ~cond:GT dst (imm 0)]
                 end
               | A.LtOp -> begin
                   [cmp dst oper;
                    mov ~cond:LT dst (imm 1);
                    mov ~cond:GE dst (imm 0)]
                 end
               | A.EqOp -> begin
                   [cmp dst oper;
                    mov ~cond:EQ dst (imm 1);
                    mov ~cond:NE dst (imm 0)]
                 end
               | A.NeOp -> begin
                   [cmp dst oper;
                    mov ~cond:NE dst (imm 1);
                    mov ~cond:EQ dst (imm 0)]
                 end)
            end
          ...
          | A.ArrayIndexExp (name, [exp]) -> begin
              ...
              let insts = translate_exp env exp (index::rest)
                          ...
                          @ [mov o (imm size);
                             mul index index o]
                          @ (if is_string t then [] else [add index index (imm 4)])
          ...
          | A.ArrayIndexExp (name, explist) -> begin
              ...
              let buffer = ref [] in
              let (index::addr::o::rest) = rest in
              let skip_length_inst = if is_string t then [] else [add index index (imm 4)] in
              buffer := !buffer @ trans_var acc addr;
              let rec emit_addressing = (function
                  | exp::other -> begin
                      buffer := !buffer @ translate_exp env exp (index::rest);
                      buffer := !buffer @ [mov o (imm size); mul index index o]
                                @ skip_length_inst
                                @ [add addr addr (OperReg (index, None));
...
and translate (env: E.env)
  ...
  | AssignStmt   (lhs, rhs) -> begin
      ...
      let other::_ = rest in
      let check_null_insts = [add other b (imm offset)] @ trans_call "wacc_check_pair_null" [other] in
  ...
  | VarDeclStmt  (ArrayTy ty, name, (LiteralExp(LitArray elements),_)) -> begin
      ...
      let insts = [MOV (dst, imm (element_size * array_length + 4)), None] @
      ...
      let insts = insts @ [mov next (imm array_length);
  ...
  | PrintStmt    (newline, exp) -> begin
      ...
      let ci = trans_call ("wacc_print_" ^ (string_of_ty ty)) [dst] in
  ...
  | RetStmt      (exp) -> begin
      ...
      inst_list @ [mov (reg_RV) (OperReg (dst, None));
                   ...
                   add reg_SP reg_SP (imm (frame_size frame));
  ...
  | ReadStmt (exp) -> begin
      ...
      let ci = trans_call ~result:dst ("wacc_read_" ^ (string_of_ty ty)) [] in
      let ci = ci @
               (if ty = CharTy then
...
let fixup_allocation frame insts =
  let open Arm in
...
let rec translate_function_decs decs env inst_list =
  ...
  let tr_func ((A.FuncDec(retty, name, field_list, stmt),pos)) = begin
    ...
    let alloc_insts = [(sub reg_SP reg_SP (imm local_size))] in
    let dealloc_insts = [(add reg_SP reg_SP (imm local_size))] in
    let func_insts = label_inst::push_inst::push_reg::(alloc_insts@(inst_list @ insts)) in
    (fixup_allocation frame (func_insts @ dealloc_insts @ [pop_reg] @ [pop_inst] @ [(LTORG, None)]));
  end in
