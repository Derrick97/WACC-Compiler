...
module Llvm_backend = struct
  ...
  let setup_builtin_functions =
    ...
    let _ = declare_function "printf" print_fty global_module in

    let print_bool_fty = function_type i32_type (Array.of_list [(i8_type)]) in
    let _ = declare_function "print_bool" print_bool_fty global_module in
    let _ = declare_function "println_bool" print_bool_fty global_module in

    let print_char_fty = function_type i32_type (Array.of_list [(i8_type)]) in
    let _ = declare_function "print_char" print_char_fty global_module in
    let _ = declare_function "println_char" print_char_fty global_module in

    let print_string_fty = function_type i32_type (Array.of_list [(pointer_type i8_type)]) in
    let _ = declare_function "print_string" print_string_fty global_module in
    let _ = declare_function "println_string" print_string_fty global_module in

    let print_int_fty = function_type i32_type (Array.of_list [(i32_type)]) in
    let _ = declare_function "print_int" print_int_fty global_module in
    let _ = declare_function "println_int" print_int_fty global_module in

    let print_pair_fty = function_type i32_type (Array.of_list [(i8_type)]) in
    let _ = declare_function "print_pair" print_pair_fty global_module in
    let _ = declare_function "println_pair" print_pair_fty global_module in

    let print_array_fty = function_type i32_type (Array.of_list [(i8_type)]) in
    let _ = declare_function "print_array" print_array_fty global_module in
    let _ = declare_function "println_array" print_array_fty global_module in

    (* ord, chr *)
    let ord_fty = function_type i32_type (Array.of_list [i8_type]) in
    let _ = declare_function "ord" ord_fty global_module in
    let chr_fty = function_type i8_type (Array.of_list [i32_type]) in
    let _ = declare_function "chr" chr_fty global_module in
    (* len *)
    (* read, including read_int read_char separately *)
    let read_int_fty = function_type i32_type (Array.of_list []) in
    let _ = declare_function "read_int" read_int_fty global_module in
    let read_char_fty = function_type i8_type (Array.of_list []) in
    let _ = declare_function "read_char" read_char_fty global_module in
    ()
  ...
  *)
  ...
  and codegen_exp (table: Llvm.llvalue Symbol.table) (exp: A.exp)
    ...
    | IdentExp (name) -> begin
        ...
        | Integer -> table, build_load v (name) builder
    ...
    | LiteralExp (literal) ->
      begin
        ...
        | LitNull -> table, const_int i32_type 0
      end
  ...
  and codegen_decl (FuncDec (ty, name, args, body), _) =
    ...
    f
  and codegen_stmt (table: Llvm.llvalue Symbol.table) (exp: A.stmt)
    : (Llvm.llvalue Symbol.table * Llvm.llvalue) =
    let exp, pos = exp in
    match exp with
    | ExitStmt (exp) -> table, build_ret_void builder
    | RetStmt  (exp) -> (let (_, exp') = codegen_exp table exp in
                              table, build_ret exp' builder)
    | ReadStmt ((A.IdentExp (_), _) as exp) -> codegen_read table exp
    | ReadStmt (_, _) -> raise (Error "Cannot read to anything other than var")
    | FreeStmt (exp) -> begin
        let (_, inst) = codegen_exp table exp in
        table, build_free inst builder
      end
    | AssignStmt   ((IdentExp (name), pos), rhs) ->
      begin
    ...
    | AssignStmt ((ArrayIndexExp (name, idxs), _) , rhs) ->
      begin
    ...
    | AssignStmt ((SndExp (IdentExp lhs, _), _), rhs) ->
      begin
        let (table, instr) = codegen_exp table rhs in
    ...
    | AssignStmt (_, _) -> (invalid_arg "assignment of non-identity not allowed")
    (* Declare an array from array literal*)
    | VarDeclStmt   (ArrayTy ty, name, exp) ->
      begin
    ...
    | SkipStmt -> table, const_string context "skip" (* dummy for type coherence *)
    | PrintStmt (_, exp) -> table, codegen_print table exp
    ...
    | BlockStmt (body) -> (let table = Symbol.new_scope table in codegen_stmt table body)
    | SeqStmt (stmt, stmtlist) -> (let (table, _) = codegen_stmt table stmt in
