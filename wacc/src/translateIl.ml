  ...
  begin match exp with
    ...
    | A.BinOpExp      ((lhs), binop, (rhs)) -> begin
        ...
        (match binop with
            ...
            | A.DivideOp -> emit (div dst opl opr)
    ...
    | A.UnOpExp       (unop, exp) -> begin
        ...
        (match unop with
            | A.NotOp -> emit (eor dst (oper_reg dst) (oper_imm 1))
            | A.NegOp -> emit (sub dst (oper_imm 0) opt)
...
and addr_of_exp (env) (e: A.exp): (Il.addr) =
  ...
  | FstExp (exp) -> begin
      let dst = allocate_temp() in
      let next = trans_exp env exp in
      let _ = trans_call env "wacc_check_pair_null" [dst] in
      emit (load WORD next (addr_indirect dst 0));
      (addr_indirect next 0)
    end
  | SndExp (exp) -> begin
      let dst = allocate_temp() in
      let next = trans_exp env exp  in
      let _ = trans_call env "wacc_check_pair_null" [dst] in
      emit(load WORD next (addr_indirect dst 4));
      (addr_indirect next 0)
    end
...
and trans_prog (ctx:ctx) (decs, stmt) (out: out_channel) = begin
  ...
  (* Array.iter (fun i -> print_endline (IL.show_il i)) insts; *)
  (* build CFG *)
